...
static int 
session_conn_shutdown(iscsi_conn_t *conn, queue_task_t *override_qtask,
		      int err)
{
	iscsi_session_t *session = conn->session;
	queue_task_t *running_qtask = NULL;

	if (conn->login_qtask) {
		running_qtask = conn->login_qtask;
		conn->login_qtask = NULL;
	} else if (conn->logout_qtask) {
		running_qtask = conn->logout_qtask;
...
cleanup:
	...
	if (running_qtask)
		mgmt_ipc_write_rsp(running_qtask, err);
	if (override_qtask)
...
static void iscsi_login_eh(struct iscsi_conn *conn, int err)
{
	...
	case ISCSI_CONN_STATE_XPT_WAIT:
		...
		case R_STAGE_NO_CHANGE:
			...
			if (iscsi_login_is_fatal_err(err) ||
				...
				session_conn_shutdown(conn, NULL, err);
			else {
	...
	case ISCSI_CONN_STATE_IN_LOGIN:
		...
		case R_STAGE_SESSION_REOPEN:
			...
			break;
		case R_STAGE_SESSION_CLEANUP:
			session_conn_shutdown(conn, NULL,
...
setup_full_feature_phase(iscsi_conn_t *conn)
{
	...
	if (session->r_stage == R_STAGE_NO_CHANGE ||
		...
		session_rescan_host(session, conn, conn->login_qtask);
	}
	conn->login_qtask = NULL;
...
static void iscsi_stop(void *data)
{
	...
	rc = session_conn_shutdown(conn, NULL, ISCSI_SUCCESS);
	if (rc)
		log_error("BUG: Could not shutdown session:%d.", sid);
}
...
static void session_conn_process_login(void *data)
{
	...
	if (session->r_stage == R_STAGE_NO_CHANGE ||
		 ...
		 * don't want to re-scan it on recovery.
		 */
		session_scan_host(session, session->hostno, conn->login_qtask,
				  false);
...
int session_logout_task(int sid, queue_task_t *qtask)
{
	...
	/*
	 * We checked for the initial login above, so if login_qtask is set
	 * then it's for a sync.
	 */
	if (conn->login_qtask) {
		sess_warn(session, "Forcing removal of syncing session.");
		return session_conn_shutdown(conn, qtask, ISCSI_SUCCESS);
	}
